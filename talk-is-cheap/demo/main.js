...
// 格式化日对象
        ...
        var login = function () {
            ...
            if (roomId) {
                yim.joinRoom(roomId).catch(function (e) {
                    ...
                    //登录成功后，加载历史消息记录
                    queryRoomHistoryMessage(roomId,30,0,'');//923730693861945349
                });
                curRoomId = roomId;
            }
         ...
         * 获取历史消息记录
         * @param roomId 频道ID
         * @param count 消息数量(单次最大返回30条，最大保存200条)
         * @param directon 历史消息排序方向 0：按时间戳升序 1：按时间戳逆序
         * @param lastMsgid 上一分页最后last_msgid值,第一页可传空
         */
        function queryRoomHistoryMessage(roomId,count= 30,directon= 0,lastMsgid= '') {
            lastMsgid = lastMsgid.toString();

                       yim.queryRoomHistoryMessageByLastMsgid(roomId, count, directon , lastMsgid).then(function(msgList) {
